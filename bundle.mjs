...
var SOCKET_PATH = "/tmp/_primordial_delegate.sock";
...
var FACTS_FILE = join(MEMORY_DIR, "facts.json");
function initMemory() {
...
function loadFacts() {
  try {
    return JSON.parse(readFileSync(FACTS_FILE, "utf-8"));
  } catch {
...
function getContext() {
  ...
  if (recent.length) {
    ...
    for (const c2 of recent) {
      parts.push(`[${c2.timestamp}] User: ${c2.user.slice(0, 150)}`);
      parts.push(`Response: ${c2.assistant.slice(0, 200)}`);
    }
  }
  const facts = loadFacts();
  if (facts.facts?.length) {
    parts.push("\n### Known facts");
    for (const f2 of facts.facts) parts.push(`- ${f2}`);
  }
...
function searchMemory(query) {
  ...
  const results = [];
  for (const c2 of getRecentConversations(50)) {
    if ((c2.user + c2.assistant).toLowerCase().includes(q2)) {
      results.push(`[${c2.timestamp}] User: ${c2.user.slice(0, 100)} \u2192 Response: ${c2.assistant.slice(0, 100)}`);
    }
  }
  const facts = loadFacts();
  for (const f2 of facts.facts || []) {
    if (f2.toLowerCase().includes(q2)) results.push(`Fact: ${f2}`);
  }
  for (const [name, info] of Object.entries(facts.entities || {})) {
    const infoStr = typeof info === "string" ? info : JSON.stringify(info);
    if ((name + infoStr).toLowerCase().includes(q2)) results.push(`Entity: ${name} \u2014 ${infoStr}`);
  }
...
async function* socketStream(msg) {
  ...
  const queue = [];
  let resolve;
  let waiting = new Promise((r2) => resolve = r2);
  let ended = false;
  const pushLine = (line) => {
    try {
    ...
    resolve();
    waiting = new Promise((r2) => resolve = r2);
  };
  let buf = "";
  sock.on("data", (chunk) => {
    buf += chunk.toString();
  ...
  while (true) {
    ...
    for (const block of resp.content) {
      if (block.type === "tool_use") {
        ...
        const q2 = args.query || args.message || args.agent_url || args.session_id || "";
        send({ type: "activity", tool: block.name, description: q2 ? `${block.name}(${q2})` : block.name, message_id: messageId });
      }
    }
    ...
    const toolUseBlocks = resp.content.filter((b2) => b2.type === "tool_use");
    const settled = await Promise.allSettled(
      toolUseBlocks.map(async (block) => {
        ...
        try {
          ...
          return { id: block.id, content: `Error: ${e2.message}` };
        }
      })
    );
    const toolResults = settled.map((s2) => {
      const val = s2.status === "fulfilled" ? s2.value : { id: "unknown", content: `Error: ${s2.reason}` };
